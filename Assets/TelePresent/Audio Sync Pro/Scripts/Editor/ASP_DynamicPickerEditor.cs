...
namespace TelePresent.AudioSyncPro
{
    public class ASP_DynamicPickerEditor
    {
        ...
        {
            for (int i = dynamicPickers.Count - 1; i >= 0; i--) // Iterate backwards to avoid issues while deleting
            {
                var dynamicPicker = dynamicPickers[i];
                EditorGUILayout.Space();
                DrawDynamicPickerInspector(dynamicPicker, i, dynamicPickers, audioSourcePlus); // Pass the audioSourcePlus instance
            }
        }
        ...
        private void DrawDynamicPickerInspector(ASP_DynamicPicker dynamicPicker, int index, List<ASP_DynamicPicker> dynamicPickers, AudioSourcePlus audioSourcePlus)
        {
            ...
            try
            {
                ...
                if (GUILayout.Button("X", GUILayout.Width(20), GUILayout.Height(20)))
                {
                    if (index >= 0 && index < dynamicPickers.Count)
                    {
                        ...
                        GUIUtility.ExitGUI(); // Ensure immediate exit to prevent further GUI processing
                    }
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.EndVertical(); // Ensure EndVertical is called
                    return;
                }
                ...
                if (dynamicPicker.IsFoldedOut)
                {
                    ...
                    if (GUI.changed) // Only record the object if something has actually changed
                    {
                    ...
                    if (dynamicPicker.selectedGameObject != null)
                    {
                        ...
                        if (newSelectedComponentIndex != selectedComponentIndex)
                        {
                            ...
                            if (target != null)
                            {
                                ...
                                if (methods.Length > 0)
                                {
                                    ...
                                    // Initialize method parameters for the first method
                        ...
                        if (targetForMethods != null)
                        {
                            ...
                            var methodNames = System.Array.ConvertAll(methods, method => method.Name);
                            int selectedMethodIndex = System.Array.IndexOf(methodNames, dynamicPicker.selectedMethodName);
                            ...
                            if (newSelectedMethodIndex >= 0 && newSelectedMethodIndex < methods.Length)
                            {
                                ...
                                if (!string.IsNullOrEmpty(dynamicPicker.selectedMethodName))
                                {
                                    ...
                                    // Ensure the methodParameters array has the correct size
                                    if (dynamicPicker.methodParameters == null || dynamicPicker.methodParameters.Length != parameters.Length)
                                    {
                                        ...
                                        for (int i = 0; i < parameters.Length; i++)
                                        {
                                            dynamicPicker.methodParameters[i] = new ASP_SerializedParameter(); // Ensure initialization
                                        }
                                        EditorUtility.SetDirty(audioSourcePlus);
                                    }
            ...
            {
                throw; // Re-throw to let Unity handle it
            }
            catch (Exception e)
            {
        ...
        private void DrawParameterField(System.Reflection.ParameterInfo parameter, ASP_SerializedParameter parameterProp, AudioSourcePlus audioSourcePlus)
        {
            ...
            // Register the undo state before modifying the parameters
            Undo.RecordObject(audioSourcePlus, "Change Method Parameter");
            ...
            else if (parameterType.IsEnum)
            {
                ...
                if (enumIndex < 0) enumIndex = 0; // Default to first entry if invalid
            ...
            if (EditorGUI.EndChangeCheck()) // If a change was detected
            {
                EditorUtility.SetDirty(audioSourcePlus);
            }
        }
