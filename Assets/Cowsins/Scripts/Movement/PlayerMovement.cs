    ...
    public class PlayerMovement : MonoBehaviour, IPlayerMovementStateProvider, IPlayerMovementActionsProvider, IPlayerMovementEventsProvider
    {
       ...
       "       if false: Speed while shooting = walkSpeed")]
        ...
        private void Start()
        {
            ...
            InputManager.inputManager.SetPlayerInputModes(alternateSprint, alternateCrouch);
        ...
        public void HandleVelocities()
        {
            ...
            if ((InputManager.sprinting || autoRun) && enoughStaminaToRun)
            {
                ...
                if (canRun)
                {
                    bool movingForward = Vector3.Dot(orientation.Forward, rb.velocity) > 0;
                    ...
                    if (forwardAllowed && sidewaysAllowed && shootingAllowed)
                    {
                        ...
                        if (currentSpeed != runSpeed && !wallRunning && rb.velocity.magnitude > .1f)
        ...
        private Vector2 FindVelRelativeToLook()
        {
            ...
            Vector3 localVel = Quaternion.Euler(0, -orientation.Yaw, 0) * rb.velocity;
        ...
        public void Movement()
        {
            ...
            if (IsPlayerOnSlope())
            {
                ...
                rb.useGravity = false;
                if (rb.velocity.y > 0) rb.AddForce(Vector3.down * 150);
            }
            else
            {
        ...
        public void HandleStairs(Vector3 moveDirection)
        {
            ...
            {
                if(moveDirection == Vector3.zero && rb.velocity.y < 0 && grounded)
                {
            ...
            else
            {
                ...
                else if(!isCrouching)
                {
                    ...
                    SnapStairsPosition(capsuleBottom, 0, 0, maxStepDistance * 2, 0);
                }
                isSteppingStairs = false;
            }
        }
        ...
        {
            return isCrouching && rb.velocity.magnitude >= crouchSpeed;
        }
        ...
        public void Jump()
        {
            ...
            else
            {
                ...
                {
                    if (Vector3.Dot(rb.velocity, new Vector3(inputX, 0, inputY)) > .5f)
                        rb.velocity /= 2f;
        ...
        public void HandleSpeedLines()
        {
            ...
            if (rb.velocity.magnitude >= minSpeedToUseSpeedLines)
            {
        ...
        public void FootSteps()
        {
            ...
            if (!grounded && !wallRunning || rb.velocity.sqrMagnitude <= .1f)
            {
